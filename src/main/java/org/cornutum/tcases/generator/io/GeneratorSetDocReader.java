...
public class GeneratorSetDocReader extends DefaultHandler implements IGeneratorSetSource
  ...
  protected class VarHandler extends ElementHandler
    ...
    public void startElement( String uri, String localName, String qName, Attributes attributes) throws SAXException
      ...
      try
        {
        parent.getTuple().addVarBinding( new VarBinding( requireAttribute( attributes, NAME_ATR), attributes.getValue("valuetype"), attributes.getValue("teststatus"), attributes.getValue("testimportance"), attributes.getValue("expectedresult"), attributes.getValue("vartype"), attributes.getValue("varsignificance"), requireAttribute( attributes, VALUE_ATR)));
        }
      catch( Exception e)
        {
